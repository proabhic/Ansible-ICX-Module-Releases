...
description:
  - This module provides declarative management of static
    IPV6 routes on Ruckus ICX network devices.
notes:
...
def shorten_ip(want):
    # unicode = str
    for item in want:
        item['prefix'] = str(ip_interface(unicode(item['prefix'])).network.compressed)
        item['next_hop'] = str(IPv6Address(unicode(item['next_hop'])).compressed)
        # result['next_hop'] = item['next_hop']
    return want
